...
const {
    timestampOfTodaysDate,
    timestampfromFormInputDate,
    timestampOfNow,
    timestampToDateAndTime,
    logger,
    updateRep,
} = require("./myModules/functions.js");
...
if (process.env.HTTPS === "on") {
    https
        ...
        .listen(process.env.HTTPS_LISTEN_PORT, () => {
            console.log(
                "HTTPS Server running on port",
                process.env.HTTPS_LISTEN_PORT
            );
        });
}
...
app.get("/production", (req, res, next) => {
    ...
    db.all(
        `SELECT * FROM jobs WHERE bookindate = ? ORDER BY duedate ASC `,
        [searchDate],
        (err, row) => {
...
app.post("/ink", logger, (req, res) => {
    if (req.body.add) {
        ...
        req.files.uploadFile.mv(
            __dirname + "/public/ink/" + req.files.uploadFile.name
        );
        db.run(
            `INSERT INTO inkdata (filename, note) VALUES (?,?)`,
            [req.files.uploadFile.name, req.body.note],
            (err) => {
                if (err) return next(err);
                res.redirect("back");
            }
...
app.post("/admin/machines", logger, userLevel("admin"), (req, res) => {
    if (req.body.add) {
        db.run(
            `INSERT INTO machines (name, printheads, dryers) VALUES (?,?,?)`,
            [req.body.machine, req.body.printheads, req.body.dryers],
            (err) => {
                if (err) return next(err);
                res.redirect("back");
            }
...
app.post("/admin/reps", logger, userLevel("admin"), (req, res) => {
    if (req.body.add) {
        db.run(
            `INSERT INTO reps (name, email) VALUES (?,?)`,
...
app.post("/admin/users", logger, userLevel("admin"), (req, res) => {
    if (req.body.add) {
        bcrypt.hash(req.body.password, 10, (err, hash) => {
            if (err) return next(err);
            db.run(
                `INSERT INTO users (user,password, email, department, userlevel) VALUES (?,?,?,?,?)`,
                [
                    req.body.username,
                    hash,
                    req.body.email,
...
app.post(
    ...
    async (req, res, next) => {
        ...
        if (Array.isArray(req.files.files)) {
            // forEach will only work on arrays so make sure it is. is there a better way to do this?
        ...
        await im.convert(
            [
                __dirname + "/public/Files_Images/" + picsArray[0].name,
                "-resize",
                "320x240",
                __dirname + "/public/Files_Images.tn/" + picsArray[0].name,
            ],
            (err) => {
        ...
        db.run(
            `INSERT INTO samples (name, number, date, otherref, printdata, printdataback, printdataother, notes, printer, pics) VALUES (?,?,?,?,?,?,?,?,?,?)`,
            [
                req.body.jobname,
                ...
                req.body.notes,
                req.session.userName,
                JSON.stringify(picNames),
            ],
            async (err) => {
                if (err) return next(err);
                const lastID = await runSQL(
                    "SELECT rowid FROM samples ORDER BY rowid DESC LIMIT 1",
                    [],
                    next
                );
...
app.post("/samples/search", (req, res, next) => {
    db.all(
        `SELECT * FROM samples WHERE name LIKE ? OR number LIKE ? OR otherref LIKE ? ORDER BY date DESC`,
        [
...
app.get("/samples/edit/:id", department("print"), (req, res, next) => {
    db.get(
        "SELECT * FROM samples WHERE rowid=?",
        [req.params.id],
        (err, row) => {
            if (err) return next(err);
            if (
                req.session.userName == row.printer ||
...
app.post("/samples/edit/:id", logger, department("print"), (req, res, next) => {
    ...
    if (req.body.submit) {
        db.get(
            `SELECT * FROM samples WHERE rowid=${req.params.id}`,
            (err, row) => {
                if (err) return next(err);
                if (
                    req.session.userName == row.printer ||
                    ...
                    db.run(
                        "UPDATE samples SET name = ?, number = ?, otherref = ?, printdata = ?, printdataback = ?, printdataother = ?, notes = ? WHERE rowid=?",
                        [
                            req.body.jobname,
                            req.body.ordernumber,
                            req.body.otherref,
...
app.post("/orders/add", logger, userLevel("admin"), (req, res, next) => {
    if (req.body.addjob) {
        db.run(
            "INSERT INTO jobs ( print,emb,transfer,aspre,digi,sample,swatch,ordernumber,ordername,takendate,duedate,machine,pos,screens,stitchcount,date,byuser,json,notes,complete,deliverytime ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            [
                req.body.print,
                req.body.emb,
                req.body.transfer,
                ...
                req.body.ordernumber,
                req.body.ordername,
                new Date(req.body.takendate).getTime() / 1000,
                new Date(req.body.duedate).getTime() / 1000,
                "unallocated",
                req.body.pos,
                req.body.screens,
                req.body.stitchcount,
                Date.now() / 1000,
                req.session.userName,
...
app.post(
    ...
    async (req, res, next) => {
        if (req.body.edit) {
            db.run(
                "UPDATE jobs set 'print' = ? ,'emb' = ?,'transfer' = ?, 'aspre' = ?, 'digi' = ?, 'sample' = ?, 'swatch' = ?, 'ordernumber' = ?, 'ordername' = ?, 'takendate' = ?,'duedate' = ?,'pos' = ?,'screens' = ?,'hasscreens' = ?,'hasstock' = ?,'hasapp' = ?,'stitchcount' = ?, 'ispacked' = ?, 'deliverytime' = ? WHERE id=?",
                [
                    req.body.print,
                    req.body.emb,
                    req.body.transfer,
        ...
        if (req.body.addStock) {
            var result = await runSQL(
                "SELECT garments FROM jobs WHERE id=?",
                [req.params.id],
                next
            );
            try {
                ...
                // if garments not valid json eg. when no garments exist
                var garments = [];
                garments.push({
                    code: req.body.code,
                    colour: req.body.colour,
            ...
            db.run(
                "UPDATE jobs SET garments = ? WHERE id=?",
                [JSON.stringify(garments), req.params.id],
                (err) => {
                    if (err) return next(err);
                }
            );
        ...
        if (req.body.deleteStock) {
            ...
            try {
                ...
                var garments = [];
                console.error("delete garments error in parse");
            }
            var newGarments = [];
            for (var i = 0; i < garments.length; i++) {
                if (req.body.deleteStock == i) {
                    continue;
                }
            ...
            db.run(
                "UPDATE jobs SET garments = ? WHERE id=?",
                [JSON.stringify(newGarments), req.params.id],
                (err) => {
                    if (err) return next(err);
                }
            );
        ...
        if (req.body.addNotes) {
            db.run(
                ...
                [req.body.notes.trim(), req.params.id],
                (err) => {
                    if (err) return next(err);
                }
            );
        ...
        if (req.body.bookIn) {
            db.run(
                "UPDATE jobs SET bookindate = ?, machine = ? WHERE id=?",
                [
                    timestampfromFormInputDate(req.body.bookInDate),
                    req.body.machine,
                    req.params.id,
                ],
                (err) => {
            ...
            var job = await runSQL(
                "SELECT rep, emailrep FROM jobs WHERE id=?",
                [req.params.id],
                next
            ); // get job options
            if (job[0].emailrep === "on") {
                updateRep(
                    job[0].rep,
                    req.params.id,
                    `Order booked in for production ${timestampToDateAndTime(
                        timestampfromFormInputDate(req.body.bookInDate)
                    )}`,
                    next
                ); //send email
        ...
        if (req.body.options) {
            db.run(
                "UPDATE jobs SET rep = ?, emailrep = ? WHERE id=?",
                [req.body.rep, req.body.email, req.params.id],
                (err) => {
                    if (err) return next(err);
                }
            );
...
app.get("/orders/search", async (req, res, next) => {
    ...
    if (req.query.search) {
        if (req.query.all) {
            // show all jobs
            db.all(
                "SELECT * FROM jobs WHERE ordername LIKE ? OR ordernumber LIKE ? ORDER BY duedate ASC",
                [`%${req.query.search}%`, `%${req.query.search}%`],
                (err, row) => {
                    if (err) return next(err);
                    res.render("orders/search.ejs", {
                        row,
                        session,
                        darkmode,
                        machines,
            ...
            // show only incomplete jobs
            db.all(
                "SELECT * FROM jobs WHERE (ordername LIKE ? OR ordernumber LIKE ?) AND complete='0' ORDER BY duedate ASC",
                [`%${req.query.search}%`, `%${req.query.search}%`],
                (err, row) => {
                    if (err) return next(err);
                    res.render("orders/search.ejs", {
    ...
    if (req.query.select) {
        db.all(
            "SELECT * FROM jobs WHERE complete='0' AND  machine=? ORDER BY duedate ASC ",
            [req.query.select],
            (err, row) => {
                if (err) return next(err);
                res.render("orders/search.ejs", {
    ...
    // if no options given
    db.all(
        "SELECT * FROM jobs WHERE complete='0' ORDER BY duedate ASC",
        (err, row) => {
            if (err) return next(err);
            res.render("orders/search.ejs", {
                row,
...
app.get("/orders/view/:id", async (req, res, next) => {
    ...
    db.get(
        ...
        async (err, row) => {
            if (err) return next(err);
            const sampleID = await runSQL(
                "SELECT rowid FROM samples WHERE number =?",
                [row?.ordernumber],
                next
            );
            const debugVar = debug;
            res.render("orders/view.ejs", {
                row,
                session,
                darkmode,
                sampleID,
...
app.post(
    ...
    async (req, res, next) => {
        if (req.body.update) {
            db.run(
                "UPDATE jobs SET notes = ?, hasstock = ?, hasscreens = ?, hasapp = ?, ispacked = ? WHERE rowid=?",
                [
                    req.body.notes.trim(),
                    req.body.hasstock,
                    req.body.hasscreens,
        ...
        if (req.body.completejob) {
            //complete job
            db.run(
                "UPDATE jobs SET completed = ?, complete = ?, completeby = ? WHERE rowid=?",
                [timestampOfNow(), "on", req.session.userName, req.params.id],
                (err) => {
                    if (err) return next(err);
                    io.emit("refresh", req.params.id); // refresh clients
                    res.redirect("back");
            ...
            const completedJob = await runSQL(
                "SELECT rep, emailrep FROM jobs WHERE id =?",
                [req.params.id],
                next
            );
            if (completedJob[0].emailrep === "on")
                updateRep(
                    completedJob[0].rep,
                    req.params.id,
                    "Order complete",
...
app.get("/stores/stock-out/", (req, res, next) => {
    ...
    db.all(
        "SELECT * FROM goodsout WHERE number LIKE ? OR name LIKE ? ",
        [`%${req.query.search}%`, `%${req.query.search}%`],
        (err, row) => {
            if (err) return next(err);
...
app.post(
    ...
    async (req, res, next) => {
        if (req.body.remove) {
            // remove stock from stores
            const result = await runSQL(
                "SELECT id FROM goodsout WHERE number LIKE ?",
                [`%${req.body.ordernumber}%`],
                next
            );
            ...
            if (result.length != 0) {
                res.send(
                    `<script>alert('already removed form stores');window.location.replace('/stores/stock-out?search=${req.body.ordernumber}');</script>`
                );
                return;
            }

            db.run(
                `INSERT INTO goodsout (name, number, date, removedby, complete) VALUES (?,?,?,?,?)`,
                [
                    req.body.ordername,
                    req.body.ordernumber,
                    timestampOfNow(),
                ...
                (err) => {
                    if (err) return next(err);
                    res.redirect(
                        `/stores/stock-out?search=${req.body.ordernumber}`
                    );
                }
            );
        }
...
app.get("/stores/search/", (req, res, next) => {
    //  if (req.query.search == "") { req.query.search = " " }  // make empty search return all
    db.all(
        "SELECT * FROM short WHERE ordernumber LIKE ? OR productcode LIKE ?",
        [`%${req.query.search}%`, `%${req.query.search}%`],
        (err, row) => {
            if (err) return next(err);
            req.session.search = req.query.search;
...
app.post("/stores/search", logger, department("stores"), (req, res, next) => {
    ...
    if (req.body.add) {
        // add short
        db.run(
            `INSERT INTO short (ordernumber, productcode, size, colour, amount, date) VALUES (?,?,?,?,?,?)`,
            [
    ...
    if (req.body.edit) {
        // edit short
        db.run(
            "UPDATE short SET ordernumber = ?, productcode = ?, size = ?, colour = ?, amount = ?, date = ? WHERE id=?",
            [
                req.body.ordernumber.trim(),
                req.body.productcode.trim(),
                req.body.size.trim(),
...
app.use((err, req, res, next) => {
    ...
    res.send(
        `Something broke!<br>${err}<br>if the problem persists contact your administrator `
    );
